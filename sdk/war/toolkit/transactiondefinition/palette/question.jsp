...
<c:set var="questionNumberPath" value="${path}.@questionNumber"/>
...
<div id="${path}_palette" class="palette">
	...
	<tk:tabContent tabName="Options" path="${path}">
		...
		<input type="hidden" name="${path}.@type" id="${path}.@type" value="question"/>

		<tk:textarea path="${path}.@title" 
			label="Label" 
			fieldValue="${tk:getFieldValue(toolkitData,labelPath,'New Question Field')}"  
			required="*" 
			defaultValue="New Question Field" 
			size="20"
		...
		<tk:input path="${questionNumberPath}" 
			label="Question Number" 
			fieldValue="${tk:getFieldValue(toolkitData,questionNumberPath,'')}"  
			required="" 
			defaultValue="" 
			className="numeric"
			size="8">
