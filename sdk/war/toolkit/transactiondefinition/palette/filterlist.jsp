...
<div id="${path}_palette" class="palette">
	...
	<tk:tabs path="${path}">
		...
		<tk:tab tabName="Values" focusOnLoad="false" path="${path}"></tk:tab>
	...
	<tk:tabContent tabName="Options" path="${path}">
		...
		<input type="hidden" name="${path}.@type" id="${path}.@type" value="filterlist"/>

		<tk:textarea path="${labelPath}" 
			label="Label" 
			fieldValue="${tk:getFieldValue(toolkitData,labelPath,'New Filterlist Field')}"  
			required="*" 
			defaultValue="" 
			size="20"
		...
		<tk:input path="${sizePath}" 
			label="Size Of Field" 
			fieldValue="${tk:getFieldValue(toolkitData,sizePath,'20')}"  
			required="*" 
			defaultValue="" 
			size="8"
	...
	<tk:tabContent tabName="Values" path="${path}" styleValue="display: none">
		<jsp:include page="../palette/optionLists/optionLists.jsp" />
	</tk:tabContent>
