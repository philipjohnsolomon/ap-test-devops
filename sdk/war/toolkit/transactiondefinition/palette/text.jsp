...
<c:set var="sizePath" value="${path}.@size"/>
<c:set var="hopelessPath" value="${path}.@hopeless"/>
<c:set var="styleClassPath" value="${path}.@styleClass"/>
<c:set var="formatMaskTypePath" value="${path}.formatMask.@type"/>
<c:set var="formatMaskPath" value="${path}.formatMask"/>
<c:set var="formatMaskType" value="${tk:getFieldValue(toolkitData, formatMaskTypePath, '')}"/>
<c:set var="showHideDivMask" value="display: none;"/>
<c:set var="showHideDivNumeric" value="display: none;"/>
<c:set var="customMaskPath" value="${formatMaskPath}"/>
<c:set var="customNumericPath" value="${formatMaskPath}"/>
<c:set var="htmlDefinitionIdPath" value="${path}.@htmlDefinitionId"/>
<c:set var="formatMaskDeleteValue" value="true"/>

<c:if test="${formatMaskType == 'customMask'}">	
	<c:set var="showHideDivMask" value=""/>
	<c:set var="customNumericPath" value=""/>
</c:if>

<c:if test="${formatMaskType == 'customNumeric'}">	
	<c:set var="showHideDivNumeric" value=""/>
	<c:set var="customMaskPath" value=""/>
</c:if>

<c:if test="${not empty formatMaskType}">
	<c:set var="formatMaskDeleteValue" value="false"/>
</c:if>
...
<div id="${path}_palette" class="palette">
	...
	<tk:tabContent tabName="Options" path="${path}">
		...
		<input type="hidden" name="${path}.@type" id="${path}.@type" value="text"/>
		
		<tk:input path="${labelPath}" 
			label="Label" 
			fieldValue="${tk:getFieldValue(toolkitData,labelPath,'New Text Field')}"  
			required="*" 
			defaultValue="" 
			size="40"
	...
	<tk:tabContent tabName="Impacted By" path="${path}" styleValue="display: none">	
		<tk:displayPageSearch errorMsg="">
			<jsp:include page="behaviorCrossRef.jsp">
				<jsp:param name="entity" value="Field Element"></jsp:param>
	...
	<tk:tabContent tabName="Advanced" path="${path}" styleValue="display: none">	
		...
		<input type="hidden" name="${formatMaskPath}.@delete" id="${formatMaskPath}.@delete" value="${formatMaskDeleteValue}" />						
		...
		<tk:input path="${sizePath}" 
			label="Size Of Field" 
			fieldValue="${tk:getFieldValue(toolkitData,sizePath,'')}"  
			required="" 
			defaultValue="" 
			size="10"
			elementName="${sizePath}"
			className="numeric" 
			helptext="fieldElement_helptext|sizeoffield">
		</tk:input>	
		...
		<tk:select label="Format Mask" 
			path="${formatMaskTypePath}"
			className="formatMaskSelector change_event"
			helptext="fieldElement_helptext|formatMaskType">
			${tk:getOptionsHTML(toolkitData,'formatMaskTypes',formatMaskTypePath,'')}
		</tk:select>
		<tk:input path="${formatMaskPath}.customMask"
			elementName="${customMaskPath}"
			label="Custom Format" 
			fieldValue="${tk:getFieldValue(toolkitData,formatMaskPath,'')}"  
			required="*" 
			defaultValue="" 
			size="40"
			hidden="${showHideDivMask}"
			className="resetValue"
			helptext="fieldElement_helptext|customFormatMaskType">
		</tk:input>
		<tk:input path="${formatMaskPath}.customNumeric"
			elementName="${customNumericPath}"
			label="Custom Format" 
			fieldValue="${tk:getFieldValue(toolkitData,formatMaskPath,'')}"  
			required="*" 
			defaultValue="" 
			size="40"
			hidden="${showHideDivNumeric}"
			className="resetValue"
			helptext="fieldElement_helptext|customFormatMaskType">
		</tk:input>
