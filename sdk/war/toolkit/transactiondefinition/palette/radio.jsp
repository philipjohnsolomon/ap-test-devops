...
<div id="${path}_palette" class="palette">
	...
	<tk:tabContent tabName="Options" path="${path}">
		...
		<input type="hidden" name="${path}.@type" id="${path}.@type" value="radio"/>
		
		<tk:input path="${labelPath}" 
			label="Label" 
			fieldValue="${tk:getFieldValue(toolkitData,labelPath,'New Radio Field')}"  
			required="*" 
			defaultValue="" 
			size="20"
	...
	<tk:tabContent tabName="Values" path="${path}" styleValue="display: none">
		<jsp:include page="../palette/optionLists/optionLists.jsp" >
			<jsp:param name="defaultSource" value="codeListRef.xml"></jsp:param>
			<jsp:param name="defaultTarget" value="boolean"></jsp:param>
		</jsp:include>
	</tk:tabContent>
