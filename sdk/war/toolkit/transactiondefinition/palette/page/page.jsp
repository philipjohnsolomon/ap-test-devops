...
<c:set var="isSummaryPath" value="${path}.@isSummary"/>
...
<div id="${path}_palette" class="palette">
	...
	<tk:tabContent tabName="Options" path="${path}" styleValue="">
		...
		<input type="hidden" name="${path}.@type" id="${path}.@type" value="page"/>

		<tk:input path="${titlePath}"
			label="Title"
			fieldValue="${tk:getFieldValue(toolkitData,titlePath,'New Custom Page')}"
			required="*"
			defaultValue=""
			size="20"
	...
	<tk:tabContent tabName="Used By" path="${path}" styleValue="display: none">
	 ...
	 <tk:tabContent tabName="Advanced" path="${path}" styleValue="display: none">
		...
		<tk:select label="Validate Transaction On Display"
	 	...
	 	<tk:select label="Is Summary Page"
			path="${isSummaryPath}"
			helptext="page_helptext|issummary">
			${tk:getOptionsHTML(toolkitData,'booleanFalse',isSummaryPath,'')}
		</tk:select>
	 </tk:tabContent>
</div>
