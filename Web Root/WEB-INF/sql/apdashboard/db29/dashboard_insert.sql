...
INSERT INTO report_sql 
	...
	as (select cast(? as date) start_date,
			(cast(? as date)) end_date from ${db_table_prefix}t1
	union all
	select ((start_date + 1 month) - day((start_date + 1 month)) days) + 1 DAY start_date, end_date
		from date_range
		where ((start_date + 1 month) - day((start_date + 1 month)) days) + 1 DAY <= end_date 
	)
	select substr(MONTHNAME(date_range.start_date), 1, 3) || ''-'' || 
       substr(cast(YEAR(date_range.start_date) as char(4)), 3) as series,
	   ${GID} as gid, 
       ${math_function}(${VALUE}) as value 
	from date_range left join ${db_table_prefix}work_item w
	  on (w.effective_date between date_range.start_date and date_range.end_date and w.effective_date <= date_range.start_date + 1 month - day(date_range.start_date + 1 month) days
		  and w.user_group_id in (${user_groups}) and w.status <> 55 
		  and w.commit_flag <> 0)
...
INSERT INTO report_sql 
	...
	and cast(a.create_time as date) >= ? and cast(a.create_time as date) <= ?
...
INSERT INTO report_sql 
   ...
   select substr(MONTHNAME(a.start_date), 1, 3) || ''-'' || 
    substr(cast(YEAR(a.start_date) as char(4)), 3) as series,
