...
INSERT INTO report_sql 
	(sql_id, report_sql)
    VALUES (1000,
   	'with date_range (start_date, end_date)
	as (select cast(? as datetime) start_date,
			cast(? as datetime) end_date
	union all
	select dateadd(mm,1,start_date - day(start_date)+1), end_date
		from date_range
		where dateadd(mm,1,start_date - day(start_date)+1) < end_date
	)
	select substring(datename(mm, date_range.start_date), 1, 3) + ''-'' +
	   substring(cast(datepart(yy, date_range.start_date) as varchar), 3, 2) series,  
	   ${GID} as gid, 
       ${math_function}(${VALUE}) as value 
	from date_range left join ${db_table_prefix}work_item w 
	  on (date_range.start_date <= w.effective_date and date_range.end_date >= w.effective_date 
		  and dateadd(mm,1,date_range.start_date - day(date_range.start_date)+1)-1 >= w.effective_date
		  and w.user_group_id in (${user_groups}) and w.status <> 55 
		  and w.commit_flag <> 0)
...
INSERT INTO report_sql 
	(sql_id, report_sql)
    VALUES (1001,
    'select top 10 r.rule_name as series, 
	...
	and cast(a.create_time as datetime) >= ? and cast(a.create_time as datetime) <= ?
	and cast(a.user_group_id as int) in (${user_groups})
	and w.status <> 55 
    and w.commit_flag <> 0
	and w.LOB in (${LOB})
...
INSERT INTO report_sql 
  ...
  select substring(datename(mm, a.start_date), 1, 3) + ''-'' +
	   substring(cast(datepart(yy, a.start_date) as varchar), 3, 2) series , 
   ...
   select  date_range.start_date,  
	...
	group by date_range.start_date, w.${GID}
